...
from game.lookup_tables import Winner
...
class Chessboard(object):
    ...
    def is_end(self):
        ...
        if not red_king:
            ...
            for i in range(red_king.position.y + 1, black_king.position.y):
                if self.chessmans[red_king.position.x][i] is not None:
                    checking = False
                    break
            if checking:
                if self.is_red_turn:
    ...
    def print_to_cl(self, is_print=True):
        ...
        for i in range(9, -1, -1):
            for j in range(9):
                if self.__chessmans[j][i] is not None:
        ...
        for i in range(9):
            for j in range(10):
                ...
                if chess is not None:
                    ...
                    if chess is not None and chess.is_red != self.__is_red_turn:
    ...
    def check_two_chesses_in_one_row(self, chess, old_x, old_y):
        for j in range(10):
            ...
            if chs is not None and chs.is_red == chess.is_red:
        ...
        if self.winner is None:
            ...
            for move in legal_moves:
                if int(move[2]) == target.x and int(move[3]) == target.y:
                    ...
                    final_move = move
                    break
        return self.winner is not None, final_move
