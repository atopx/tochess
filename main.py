...
def load_image(file, sub_dir=None):
    ...
    if sub_dir:
        file = os.path.join(main_dir, 'asset/image', sub_dir, file)
    else:
        file = os.path.join(main_dir, 'asset/image', file)
    try:
...
def load_images(*files):
    imgs = []
    for file in files:
        imgs.append(load_image(file, "chess"))
    return imgs
...
class ChessmanSprite(pygame.sprite.Sprite):
    ...
    def move(self, col_num, row_num):
        ...
        if is_correct_position:
            self.rect.move_ip((col_num - old_col_num) * 57, (old_row_num - row_num) * 57)
...
def creat_sprite_group(sprite_group, chessmans_hash):
    for chess in chessmans_hash.values():
        ...
        chessman_sprite = ChessmanSprite(images, chess)
...
def main(winstyle=0):
    ...
    # create text label
    font_file = os.path.join(main_dir, 'asset/font/wenkai.ttf')
            ...
            if event.type == pygame.QUIT:
                ...
                for index in range(len(pressed_array)):
                    if index == 0 and pressed_array[index]:
                        ...
                        chessman_sprite = select_sprite_from_group(chessmans, col_num, row_num)
